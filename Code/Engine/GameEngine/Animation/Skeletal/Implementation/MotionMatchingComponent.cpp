...
void ezMotionMatchingComponent::OnSimulationStarted()
{
  ...
  if (m_hSkeleton.IsValid())
  {
    ...
    m_hSkinningTransformsBuffer = ezGALDevice::GetDefaultDevice()->CreateBuffer(BufferDesc,
...
ezMotionMatchingComponent::TargetKeyframe ezMotionMatchingComponent::FindNextKeyframe(
  const TargetKeyframe& current, const ezVec3& vTargetDir) const
{
  TargetKeyframe kf;
...
void ezMotionMatchingComponent::PrecomputeMotion(ezDynamicArray<MotionData>& motionData, ezTempHashedString jointName1,
  ezTempHashedString jointName2, const ezAnimationClipResourceDescriptor& animClip, ezUInt16 uiAnimClipIndex, const ezSkeleton& skeleton)
{
